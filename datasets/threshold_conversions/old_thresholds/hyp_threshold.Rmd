---
title: "New Dry Weight and Threshold conversions -- C. Hyperboreus"
output: 
  html_document:
    toc: TRUE
---
...
We'll start by working with the raw azmp and ecomon dataset. 
...
ecomon <- readr::read_csv(file.path(root, "vertical_correction_ecomon_hyp.csv.gz"))
```
```{r}
e2 <- ecomon |>
  select(sta_depth, lat, lon, CIV_CVI_m2:corrected_CIV_CVI_m2) |>
  mutate(threshold = threshold_method(sta_depth, 40000*195, 100*195),
         no_vc = CIV_CVI_m2 >= threshold, 
         vc = corrected_CIV_CVI_m2 >= threshold) |>
  filter(no_vc != vc)
```

The azmp points have some outliers. I cannot yet remove these, as I don't have access to all variables (will look later)
```{r eval = FALSE}
Data  |>
  filter(!(Tmin > 5 & Chyp_CIV_CVI > 0 & 
             (TRANSECT %in% c("BBL","HL") | STATION =="HL2" | (STATION=="Riki" & Tmin > 5))))
```
...
ae_dw_nooutliers <- data |>
  filter(!(Tbtm > 5 & abundance > 0 & src == "azmp" &
         (transect %in% c("BBL", "HL") | station %in% c("HL2", "RIM"))))

aedwn <- ae_dw_nooutliers |>
  mutate(dry_weight = abundance * 
           map(region_tc, ~ifelse(.x == "gsl", 1390, 1145)) |> unlist())

# saving to file 
readr::write_csv(aedwn, 
                 file = file.path(root, "tc_datasets/ae_dryweight_nooutliers_vc_chyp.csv.gz"))
```
```{r, echo = FALSE}
plot <- ggplot() +
  geom_polygon(data = ggplot2::map_data("world"), 
               aes(long, lat, group = group),
               fill = "lightgray", col = "gray") +
  geom_point(data = ae_dw_outliers, 
             aes(x = lon, y = lat, col = log10(abundance + 1)), alpha = .7) +
  coord_quickmap(xlim = c(-76, -40), ylim = c(35, 60), expand = TRUE) +
  theme_bw() + 
  ggtitle(paste0("AZMP outliers (n = ", nrow(ae_dw_outliers), ")"))
...
```{r}
ae <- readr::read_csv(file.path(root, "tc_datasets/ae_regionvd_chyp.csv.gz"),
...
ggplot(ae, aes(x = log10(abundance + 1))) +
  ...
  ggtitle("Abundance count distribution - C. Hyperboreus") + 
  theme_bw()
```
...
ggplot(ae_dw, aes(x = log10(dry_weight + 1))) +
  geom_histogram(bins = 50, col = "white", fill = "yellowgreen") +
  ggtitle("Dry Weight biomass distribution - Chyp") + 
  theme_bw()
```
```{r, echo = FALSE}
ae_dw |> 
  ...
  geom_raster() +
  theme_bw() +
  ggtitle("Number of points for each region and month - Chyp") +
...
ggplot(filter(ae_thresholds, patch), aes(x = lon, y = lat)) +
  ...
  coord_quickmap(xlim = c(-76, -40), ylim = c(35, 60), expand = TRUE) +
  theme_bw() + 
  ggtitle("Positive patch values - Chyp")
```
create rudimentary model, step function match original graph individuals do model run 
